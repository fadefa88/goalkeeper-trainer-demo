(() => {
  ...
  function appendTrainingExercises(list, row) {
    ...
    row.plan.items.forEach((item, index) => {
      ...
      card.innerHTML = `<div class="training-item-top"><div><p class="eyebrow">${esc(ex?.ambito || "Esercizio")} · ${Number(item.minutes || ex?.durationMin || 0)} min</p><h4>${esc(ex?.name || "Esercizio")}</h4></div><button class="primary-btn" type="button" data-training-start="1" data-plan-date="${esc(row.date)}" data-plan-index="${index}">Avvia</button></div><div class="training-item-actions"><label class="training-done"><input type="checkbox" data-training-done="1" data-plan-date="${esc(row.date)}" data-plan-index="${index}"${item.done ? " checked" : ""} /> Fatto</label></div><label class="small-note">Note esercizio<textarea data-training-note="1" data-plan-date="${esc(row.date)}" data-plan-index="${index}" placeholder="Note operative, varianti, focus tecnico...">${esc(item.note || "")}</textarea></label>`;
