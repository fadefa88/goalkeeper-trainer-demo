const CACHE_NAME = "gk-trainer-cloudflare-d1-exercise-visuals-v1";
const ASSETS = [
  "./",
  "./index.html",
  "./privacy.html",
...
;(() => {
  const VISUALS = {
    "portiere-prese-contatti": { type: "multi-goal", title: "6 porte, conduzione e presa", steps: ["Giocatori in conduzione", "Entrata frontale nelle porticine", "Presa/intervento e cambio ruolo"] },
    "re-dei-portieri": { type: "shot-rotation", title: "Tiro continuo e rotazione", steps: ["Tiro in porta", "Parata o gol", "Rotazione rapida"] },
    "conduzione-palla-uscita-bassa": { type: "square-dive", title: "Quadrato, conduzione e uscita bassa", steps: ["Compagni in conduzione", "Portiere attacca la palla", "Presa/respinta fuori dal campo"] },
    "lancio-presa-palla": { type: "triangle-pass", title: "Triangolo, lancio e presa", steps: ["Lancio verso il compagno", "Presa", "Trasmissione al laterale libero"] },
    "uscita-bassa-cross-laterale": { type: "lateral-cross", title: "Cross laterale e presa in tuffo", steps: ["Palla laterale", "Lettura della traiettoria", "Uscita bassa in presa"] },
    ...
    "portieri-partite-aeree-estese": { type: "extended-area", title: "Metà campo: tutti possono fare il portiere", steps: ["Palla nella propria metà", "Intercetto con le mani", "Ripartenza rapida"] },
  ...
  function ensureExerciseVisualStyles() {
    ...
    style.textContent = "\n" +
      ...
      ".exercise-visual-loop{position:relative;border-radius:18px;overflow:hidden;border:1px solid rgba(213,255,222,.13);background:linear-gradient(180deg,rgba(32,224,108,.10),rgba(3,20,7,.72))}\n" +
  ...
  function svgFor(type) {
    const baseStart = '<svg viewBox="0 0 360 210" role="img" aria-label="Animazione esercizio portiere"><rect x="10" y="10" width="340" height="190" rx="18" class="visual-zone"></rect>';
    const baseEnd = '</svg>';
    ...
    if (type === "multi-goal") return baseStart + '<path class="visual-goal" d="M40 45h26M40 105h26M40 165h26M294 45h26M294 105h26M294 165h26"></path>' + dot('visual-keeper anim-keeper', 85, 105, 'P') + dot('visual-opponent anim-player', 210, 132, 'G') + '<path class="visual-arrow" d="M210 132 C170 126 140 116 96 106"></path><circle class="visual-ball anim-ball-b" cx="205" cy="126" r="6"></circle>' + baseEnd;
    if (type === "shot-rotation") return baseStart + goalLeft + dot('visual-keeper anim-keeper', 70, 105, 'P') + dot('visual-opponent', 240, 105, 'T') + '<path class="visual-arrow" d="M232 104 C172 92 116 93 78 105"></path><circle class="visual-ball anim-ball-b" cx="224" cy="102" r="6"></circle><path class="visual-line" d="M245 135 C210 164 155 164 116 137"></path>' + baseEnd;
    if (type === "square-dive") return baseStart + '<rect x="84" y="44" width="190" height="122" rx="8" class="visual-line"></rect>' + dot('visual-keeper anim-keeper', 178, 105, 'P') + dot('visual-opponent anim-player', 255, 146, 'C') + '<path class="visual-arrow" d="M252 144 C218 132 205 116 184 105"></path><circle class="visual-ball anim-ball-b" cx="248" cy="140" r="6"></circle>' + baseEnd;
    if (type === "triangle-pass") return baseStart + dot('visual-keeper', 95, 150, 'P1') + dot('visual-keeper', 180, 58, 'P2') + dot('visual-keeper', 265, 150, 'P3') + '<path class="visual-arrow" d="M102 143 L172 66 L256 143 L112 150"></path><circle class="visual-ball anim-ball-c" cx="104" cy="143" r="6"></circle><circle class="visual-ball anim-ball-a" cx="178" cy="63" r="5"></circle>' + baseEnd;
    if (type === "lateral-cross") return baseStart + goalLeft + dot('visual-keeper anim-keeper', 70, 105, 'P') + dot('visual-opponent', 274, 164, 'S') + '<path class="visual-arrow" d="M270 160 C210 136 144 126 76 107"></path><circle class="visual-ball anim-ball-b" cx="260" cy="154" r="6"></circle>' + baseEnd;
    if (type === "two-zones") return baseStart + '<line x1="180" y1="22" x2="180" y2="188" class="visual-line"></line>' + goalLeft + goalRight + dot('visual-keeper anim-keeper', 62, 105, 'P') + dot('visual-keeper anim-keeper', 298, 105, 'P') + dot('visual-opponent anim-player', 130, 76, 'A') + dot('visual-opponent anim-player', 230, 136, 'A') + '<path class="visual-arrow" d="M130 78 L75 103M230 136 L294 106"></path><circle class="visual-ball anim-ball-b" cx="128" cy="75" r="5"></circle><circle class="visual-ball anim-ball-a" cx="228" cy="134" r="5"></circle>' + baseEnd;
    if (type === "long-short") return baseStart + dot('visual-player', 70, 150, 'A') + dot('visual-player anim-player', 180, 72, 'B') + dot('visual-player', 290, 150, 'C') + '<path class="visual-arrow" d="M78 144 L172 78M178 82 L122 128M126 134 L178 90M188 78 L282 144"></path><circle class="visual-ball anim-ball-c" cx="78" cy="144" r="6"></circle>' + baseEnd;
    if (type === "set-piece") return baseStart + goalLeft + dot('visual-keeper anim-keeper', 66, 105, 'P') + dot('visual-player', 138, 112, 'D') + dot('visual-opponent anim-player', 212, 98, 'A') + dot('visual-opponent', 272, 150, 'B') + '<path class="visual-arrow" d="M272 148 C232 130 196 112 152 108"></path><circle class="visual-ball anim-ball-b" cx="266" cy="146" r="6"></circle>' + baseEnd;
    if (type === "extended-area") return baseStart + '<line x1="180" y1="20" x2="180" y2="190" class="visual-line"></line>' + goalLeft + goalRight + dot('visual-keeper anim-pulse', 80, 105, 'M') + dot('visual-player', 132, 70, 'C') + dot('visual-opponent anim-player', 225, 132, 'A') + '<path class="visual-arrow" d="M225 132 C190 124 145 110 86 106"></path><circle class="visual-ball anim-ball-b" cx="220" cy="128" r="6"></circle>' + baseEnd;
    if (type === "inside-outside") return baseStart + '<rect x="92" y="42" width="176" height="126" rx="8" class="visual-line"></rect>' + dot('visual-player anim-player', 135, 100, 'I') + dot('visual-player', 225, 112, 'I') + dot('visual-keeper', 180, 28, 'E') + dot('visual-keeper', 180, 184, 'E') + '<path class="visual-arrow" d="M137 100 L180 38 L224 112 L184 176"></path><circle class="visual-ball anim-ball-c" cx="137" cy="100" r="6"></circle>' + baseEnd;
    return baseStart + goalLeft + dot('visual-keeper anim-keeper', 70, 105, 'P') + dot('visual-opponent anim-player', 246, 105, 'A') + '<path class="visual-arrow" d="M246 105 C180 95 120 100 75 105"></path><circle class="visual-ball anim-ball-b" cx="238" cy="102" r="6"></circle>' + baseEnd;
  }
...
async function appJsWithVisuals(request) {
  ...
  const content = source.includes("exercise-visual-card") ? source : source + "\n" + EXERCISE_VISUAL_PATCH;
